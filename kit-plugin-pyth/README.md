...
| `getPythOnchainPrice(priceAccountAddress)` | Read a price directly from an on-chain Pyth price account. |
...
| `postPythPriceUpdate(feedId, payer)` | Post a single pull-oracle price update on-chain; returns the temporary `PriceUpdateV2` account address. |
