// Regenerates the Codama client in src/generated/mpl_token_metadata-client from
// the on-disk Anchor IDL in idls/. Run with `npm run generate`.
//
...
const idlPath = fileURLToPath(new URL("./idls/mpl_token_metadata.json", import.meta.url));
const clientDir = fileURLToPath(new URL("./src/generated/mpl_token_metadata-client", import.meta.url));

const anchorIdl = JSON.parse(readFileSync(idlPath, "utf-8"));
const codama = createFromRoot(rootNodeFromAnchor(anchorIdl));

const scratch = mkdtempSync(join(tmpdir(), "codama-mpl-"));
try {
...
console.log(`Generated mpl_token_metadata client into ${clientDir}`);
