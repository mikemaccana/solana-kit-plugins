...
// Exercise the plugin's read paths through the Kite connection:
...
The transport answers the **read** methods Kite and the plugins use:
...
than sending through Kite — LiteSVM executes transactions synchronously and has no subscription
endpoint for Kite's send-and-confirm path. Reads then flow through the plugin under test.
